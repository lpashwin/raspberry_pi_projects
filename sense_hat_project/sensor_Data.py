...
while(True):
    pitch,roll,yaw=(ii if ii <180.0 else ii-360.0 for ii in sense.orientation.values())#rotations
    ...
    #print(type(delta))
    #outputs=(delta.total_seconds(),pitch,roll,yaw,ax,ay,az)
    outputs=(delta.total_seconds(),pitch,roll,yaw)
    print(','.join('{:10.6f}'.format(ii) for ii in outputs))
